...
- **model<num_samples_per_variable>__<num_hidden_layers>hidden_<num_nodes_per_hidden_layer>nodes_<num_epochs>epoch_<learning_rate>_<cutoff_value>** where the model is trained on samples where outputs are in the range [cutoff_value, 0] or [-20, 0] if cutoff_value is not specified. 
